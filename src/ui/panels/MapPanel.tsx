...
import { locationDescription, resourceName } from '@i18n/content';
...
export function MapPanel() {
  ...
  return (
    <div className="col gap-3">
        ...
        notes={[
          {
            id: 'map.expedition',
            when: maxRing >= 1,
            title: t('guidance.expedition.title'),
            body: t('guidance.expedition.body'),
          },
        ]}
      />
      ...
      <Panel
        title={t('map.title')}
        note={t('map.note', { ring: maxRing, known: known.length })}
        actions={
      ...
      >
        <div className="map-wrap">
          <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="map" role="img" aria-label={t('map.aria')}>
            {[1, 2, 3].map((ring) => (
              <circle
                key={ring}
                cx={CENTRE}
              ...
              return (
                <g
                  ...
                  aria-label={t('map.nodeLabel', {
                    ...
                    state: t(`map.state.${location.state}`),
                  })}
                  onKeyDown={(e) => {
        ...
        {known.length === 0 && <EmptyState>{t('map.empty')}</EmptyState>}
      </Panel>

      {selected && <LocationInspector location={selected} onScout={() => {
        const result = scout(selected.id);
        notify(
          result.ok ? t('map.studied') : t('map.nothingMore'),
          result.ok ? 'good' : 'info',
        );
...
function LocationInspector({
  ...
  return (
    <Panel title={siteName(location)} note={t(`map.state.${location.state}`)}>
      <p className="prose">
        {knowledge.archetype && archetype ? locationDescription(archetype) : t('map.unknownSite')}
      </p>
      <ul className="kv">
        <li>
          <span>{t('map.distance')}</span>
          <span className="num">
            {t('map.distanceValue', { km: location.distanceKm.toFixed(1), days: travelDays })}
          </span>
        </li>
        <li>
          ...
          <span className="num" style={{ color: dangerColour(location.danger) }}>
            {knowledge.danger
              ? t('map.dangerValue', { value: Math.round(location.danger) })
              : t('map.unsurveyed')}
          </span>
        </li>
        <li>
          <span>{t('map.richness')}</span>
          <span className="num">{knowledge.loot ? location.richness.toFixed(1) : '—'}</span>
        </li>
        <li>
          <span>{t('map.visits')}</span>
          <span className="num">{location.visits}</span>
        </li>
      </ul>

      <span className="label">{t('map.knowledge')}</span>
      <Bar value={location.knowledge / 4} colour="var(--ice)" height={5} />

      {knowledge.loot && haul.length > 0 && (
        <>
          <span className="label">{t('map.likelyHaul')}</span>
          <ul className="kv">
            {haul.map((row) => (
              <li key={row.resource}>
                <span>
                  {(() => {
                    const def = RESOURCES[row.resource as keyof typeof RESOURCES];
      ...
      <div className="row gap-2 wrap">
        ...
        <Button size="sm" tone="primary" onClick={onPlan} disabled={!reachable}>
          {reachable ? t('map.planHere') : t('map.outOfRange')}
        </Button>
      </div>
    </Panel>
  );
